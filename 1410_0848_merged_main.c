...
if (tokenflag == 'c'){
  ...
  //jobs();
    ...
    if(!(child_pid = fork())){
        ...
        globfree(& results); // i dunno if it is necessary to free memory here
        exit(0);
        }
    else{
...
void wildcard(char* input[]){
  int i;
  glob(input[0], GLOB_NOCHECK, NULL, & results);
  for(i=1;i<255;i++){
    if(input[i]==NULL){
      break;
    }
    glob(input[i], GLOB_NOCHECK |GLOB_APPEND, NULL, & results);
  }
  for(i=0;i<results.gl_pathc;i++){
  //  printf("Wildcard results %s\n",results.gl_pathv[i]);
    input[i]=results.gl_pathv[i];
  }
  return;
}
